...
_test_write_shortcut(\&write_desktop_shortcut_file, \&create_desktop_shortcut_filename, $desktop_output_dir, " !#\$&'()+,-.09;=\@AZ[]_`az{}~中", "http://www.google.com", 1);
...
_test_write_shortcut(\&write_url_shortcut_file, \&create_url_shortcut_filename, $url_output_dir, " !#\$&'()+,-.09;=\@AZ[]_`az{}~中", "http://www.google.com", 1);


SKIP: {
    if(!defined(check_install( module => 'Mac::PropertyList' ))) {
        skip ("Mac::PropertyList not installed.  Cannot test webloc functionality unless this package is installed.");
    }

    # Note that we do not test writing UTF8 URLs.  Not sure if this is supported.
    ...
    _test_write_shortcut(\&write_webloc_binary_shortcut_file, \&create_webloc_shortcut_filename, $webloc_binary_output_dir, "Google", "http://www.google.com", 1);

    my $webloc_xml_output_dir = File::Spec->catdir ( $test_output_dir, "webloc", "xml" );
    _make_path_with_error_check($webloc_xml_output_dir);
    _test_write_shortcut(\&write_webloc_xml_shortcut_file, \&create_webloc_shortcut_filename, $webloc_xml_output_dir, "Google", "http://www.google.com", 1);
}